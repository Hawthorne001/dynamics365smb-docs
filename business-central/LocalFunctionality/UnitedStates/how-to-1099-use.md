...
1. The following information is available on the document header:  
   ...
   | **Status** | Specifies the status of the document. A new document is created with the status **Open**. The status can also be one of the following:<br><br>- **Released** — set when a user releases the document. A released document can be added to an IRIS transmission and emailed to the vendor.<br>- **In Progress** — set while the document is part of an IRIS transmission that is being processed by the IRS.<br>- **Submitted** — set after the IRS accepts the transmission that contains the document.<br>- **Abandoned** — set when the document is excluded from further reporting. |
