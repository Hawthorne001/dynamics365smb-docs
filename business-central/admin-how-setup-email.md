...
Email in !INCLUDE[prod_short short.md] is only for sending messages, except with AI agents like Sales Order Agent or Payables Agent. There’s no inbox or reply access for users, but agents can receive and read emails, taking actions based on the content (like processing orders or handling invoice queries). Users review agent responses and actions in the system without managing an inbox, highlighting the difference between standard email and agent functions.
