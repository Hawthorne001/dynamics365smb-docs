...
> [!NOTE]
> The lookup on the **Table** column shows all [!INCLUDE [prod_short](includes/prod_short.md)] tables, including custom tables, that aren't yet mapped. It excludes those that already exist on your **Integration Table Mappings** page. In Business Central versions earlier than 2026 release wave 1 (v28.0), the lookup on the **Integration Table** column shows only Dataverse tables (AL Proxy Tables) that are part of out-of-the-box integrations. To add additional or custom Dataverse tables, you need help from a developer to create and deploy them through an extension. In later versions, customer Dataverse tables and fields are available for mapping.
...
> [!NOTE]
> In Business Central versions earlier than 2026 release wave 1 (v28.0), you can't map new or custom fields added to the Dataverse table. To reduce errors, the lookup for the **Integration Table Field** column filters to a list of available fields. To map fields that the filters don't include, you need help from a developer.
>
